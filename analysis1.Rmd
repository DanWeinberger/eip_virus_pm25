...
mod2 <- inla(Cases_Total~ f(t, model='rw2', scale.model=F) + pm2_5_lag1 + pm2_5_lag2 +pm2_5_lag3 +pm2_5_lag4 +pm2_5_lag5 + day_of_week , family='nbinomial', data=ds1)
...
```{r}
  ...
  basis_pm2_5_lin <-   crossbasis(ds1$pm2_5, lag=20, argvar=list(fun="lin"),
      arglag=list(fun="poly",degree=20))
  colnames(basis_pm2_5_lin) = paste0("basis_pm2_5.", colnames(basis_pm2_5_lin))


mod1_lin <-   inla(Cases_Total ~ day_of_week  + basis_pm2_5_lin + f(t, model='rw2'), family='poisson', data=ds1,
                      control.predictor = list(link = 1, compute = TRUE),
                   control.compute=list(return.marginals.predictor=TRUE),
                control.fixed = list(correlation.matrix = TRUE, 
                            prec.intercept = 1e-4, prec = 1)
               )
summary(mod1_lin)
...
## Control for calendar time instead with a month dummy

 ds2 <- ds1 %>%
   mutate(cal_month= as.factor(paste(year(date), month(date), '01', sep='-')))

 mod2_lin <-   inla(Cases_Total ~ day_of_week  + basis_pm2_5_lin +cal_month, family='poisson', data=ds2,
                      control.predictor = list(link = 1, compute = TRUE),
                   control.compute=list(return.marginals.predictor=TRUE),
                control.fixed = list(correlation.matrix = TRUE, 
                            prec.intercept = 1e-4, prec = 1)
...
mod2.coef <- mod2_lin$summary.fixed[grep('basis_pm',row.names(mod2_lin$summary.fixed)),'mean']
...
pred2.pm <- crosspred(basis_pm2_5_lin, coef=mod2.coef, vcov=mod2.vcov, at=-10:40,bylag=0.2, cumul=TRUE)


plot(pred2.pm, "overall", var=1,lag=1, col=3, ylab="logRR", ci.arg=list(density=15,lwd=2),
main="log-RR")

plot(pred2.pm, "slices", var=1, col=3, ylab="logRR", ci.arg=list(density=15,lwd=2),
main="Association with an 1 SD increase in PM2.5")
